#!/usr/bin/env python3
"""Explicit real-toolchain gate for the Synthesizable RED2 Machine.
...
MISSING_TOP_DIAGNOSTIC = (
    "syn-check: RED2 hardware top missing: persistent reducer source "
    "has no Pypeline @MAIN; refusing to validate legacy red2_step_word"
)
SYNTHESIS_SKIPPED_DIAGNOSTIC = (
    "syn-check: PipelineC skipped synthesis because no configured "
...
def _checkout_revision(root: Path) -> str:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "syn-check: cannot determine configured PipelineC checkout revision"
        )
...
def _find_emitted_top(out_dir: Path) -> Path | None:
    top_dir = out_dir / "SynthesizableRED2Machine"
    if not top_dir.is_dir():
        return None
    candidates = sorted(top_dir.glob("SynthesizableRED2Machine_*.vhd"))
    return candidates[0] if candidates else None


def _run_native_sim(checkout_root: Path | None, repo: Path) -> subprocess.CompletedProcess[str]:
    script = repo / "scripts" / "check_syn_sim.py"
    return subprocess.run(
...
def check(frontend_only: bool = False) -> int:
    ...
    if checkout_root is not None:
        ...
        if revision != PIPELINEC_TESTED_REVISION:
            print(
                "syn-check: PipelineC revision mismatch: "
    ...
    if not source.is_file():
        print(f"syn-check: processor source missing: {source}", file=sys.stderr)
        return 2
    source_text = source.read_text()
    semantics_complete = "RED2_SYNTH_SEMANTICS_COMPLETE = 1" in source_text
    ...
    with tempfile.TemporaryDirectory(prefix="asgard-synth-red2-") as tmp:
        ...
        if result.returncode != 0:
            ...
            print(
                f"syn-check: PipelineC frontend failed with exit {result.returncode}",
                file=sys.stderr,
            )
            return 4
        ...
        if emitted_top is None:
            print(
                "syn-check: PipelineC frontend exited successfully but did not "
                "emit SynthesizableRED2Machine VHDL",
                file=sys.stderr,
            )
            return 4
        print(
        ...
        if native_sim.returncode != 0:
            ...
            print(
                "syn-check: native Synthesizable RED2 parity failed with exit "
                f"{native_sim.returncode}",
                file=sys.stderr,
            )
            return 9
        ...
        if not semantics_complete:
            print(
                "syn-check: frontend accepted the hardware top, but persistent "
        ...
        if synth.returncode != 0:
            ...
            print(
                f"syn-check: synthesis failed with exit {synth.returncode}",
                file=sys.stderr,
            )
            return 6
        ...
        if not summary:
            print(
                "syn-check: synthesis exited successfully but emitted no "
