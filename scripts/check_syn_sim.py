...
from synthesizable_red2_machine.machine import (
    CMD_CLOCK,
    CMD_LOAD_CONTROL,
...
def _run_one_commit(
    machine: AbstractRED2Machine, codec: RED2ABICodec
) -> tuple[object, object]:
    encoded = codec.encode_state(machine.state)
    processor = ConcreteRED2Machine(encoded)
...
def check() -> None:
    ...
    _run_one_commit(AbstractRED2Machine(state), codec)
    ...
    result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=5))
    ...
    for parent_env, current_env, free_space, expected_entry_env in (
        ...
        reverse_app_machine = AbstractRED2Machine(
            AbstractRED2MachineState(
    ...
    collision_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    closure_base_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
            memory=[None] * 64,
            control_stack=[None] * 32,
            pc=0,
    ...
    assert expected.argcnt == 1
    result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=39))
    ...
    for chained in (False, True):
        ...
        ep_machine = AbstractRED2Machine(
            AbstractRED2MachineState(
                memory=ep_memory,
                control_stack=[None] * 32,
                pc=0,
        ...
        result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=1))
    ...
    ep_empty_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    ep_cycle_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    ep_collision_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    ep_overflow_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    for reverse_head in (0, 1):
        ...
        assert expected.prim_id == 777 and expected.fire == 9
        result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=3))
    ...
    def run_head_prim0_y(
        ...
        *,
        ...
        processor = ConcreteRED2Machine(encoded, prim0_roles=tuple(roles))
        initial_memory = tuple(encoded.memory)
    ...
    for stop_entries in (
        ...
        for index in range(max(1, len(stop_entries))):
            result = sim_call(
                SynthesizableRED2Machine, _command(CMD_NOP, address=index)
            )
    ...
    result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=2))
    assert _packed_memory_read(result) == expected.memory[2]
    result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=10))
    ...
    general_root_processor = ConcreteRED2Machine(
        general_root_inc,
    ...
    for _ in range(256):
        general_root_result = sim_call(SynthesizableRED2Machine, _command(CMD_CLOCK))
    ...
    for _ in range(64):
        allocating_result = sim_call(SynthesizableRED2Machine, _command(CMD_CLOCK))
    ...
    allocating_result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=0))
    ...
    slot_zero_oracle = ConcreteRED2Machine(
        slot_zero_case, scalar_ops=tuple(slot_zero_ops)
    )
    ...
    assert int(metadata_result.hw_fault) == HW_FAULT_ADDRESS_RANGE
    for _ in range(128):
        metadata_result = sim_call(SynthesizableRED2Machine, _command(CMD_CLOCK))
    ...
    metadata_result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=3))
    ...
    metadata_result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=0))
    ...
    for address in (3, 4, 5):
        fire_one_result = sim_call(
            SynthesizableRED2Machine, _command(CMD_NOP, address=address)
        )
        assert _packed_memory_read(fire_one_result) == fire_one_memory[address]
    fire_one_result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=0))
    ...
    def run_configured_inc(encoded):
        processor = ConcreteRED2Machine(encoded, scalar_ops=tuple(scalar_ops))
    ...
    for table_slot, (scalar_op, literal_id, input_payload, output_payload) in enumerate(
        ...
        encoded = replace(encoded, memory=tuple(memory))
        processor = ConcreteRED2Machine(encoded, scalar_ops=tuple(ops))
        assert processor.run_to_commit()
        expected = processor.checkpoint()
        assert expected.q == 7 and expected.prim_id == 0 and expected.fire == 0
        assert expected.memory[3] == abi.pack_word(
    ...
    processor = ConcreteRED2Machine(negate_ep, scalar_ops=tuple(negate_ops))
    assert processor.run_to_commit()
    expected = processor.checkpoint()
    ...
    countdown_processor = ConcreteRED2Machine(direct_countdown)
    ...
    for _ in range(8):
        countdown_result = sim_call(SynthesizableRED2Machine, _command(CMD_CLOCK))
    ...
    countdown_result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=3))
    ...
    for saved_case in (
        ...
        for control_address in range(3):
            result = sim_call(
                SynthesizableRED2Machine, _command(CMD_NOP, address=control_address)
            )
            assert _packed_control_read(result) == expected.control_stack[control_address], (
    ...
    nested_state = AbstractRED2MachineState(
    ...
    nested_oracle = ConcreteRED2Machine(nested_encoded)
    ...
    for commit_index, (expected_pc, expected_fsp, expected_c, expected_direction) in enumerate(
        ...
        nested_result = None
        for _ in range(32):
            nested_result = sim_call(SynthesizableRED2Machine, _command(CMD_CLOCK))
    ...
    lookup_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    pnp_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
            memory=pnp_memory,
            control_stack=[None] * 32,
            pc=0,
    ...
    closure_slot_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    bad_pnp_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    bridge_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    tight_bridge_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    beta_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
            memory=beta_memory,
            control_stack=[None] * 32,
            pc=0,
    ...
    for beta_phi, beta_index in ((5, 2), (1, 3)):
        ...
        app_var_machine = AbstractRED2Machine(
            AbstractRED2MachineState(
    ...
    for generic_argument in generic_bindings:
        ...
        generic_machine = AbstractRED2Machine(
            AbstractRED2MachineState(
    ...
    for bridge_argument in bridge_beta_arguments:
        ...
        bridge_beta_machine = AbstractRED2Machine(
            AbstractRED2MachineState(
    ...
    assert expected.q == 7
    assert expected.argcnt == 1
    result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=expected.env))
    assert _packed_memory_read(result) == expected.memory[expected.env]
    result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=0))
    ...
    bridge_ep_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    app_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
            memory=app_memory,
            control_stack=[None] * 32,
            pc=0,
    ...
    bridge_app_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    collision_app_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    for rblock_env, rblock_free in ((32, 32), (28, 30)):
        ...
        rblock_machine = AbstractRED2Machine(
            AbstractRED2MachineState(
    ...
    rblock_zero_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    rblock_collision_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    rblock_zero_collision_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    assert fault == abi.FAULT_GRAPH_ENV_COLLISION
    result = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=31))
    ...
    for rup_count_case in (1, 2):
        ...
        rup_machine = AbstractRED2Machine(
            AbstractRED2MachineState(
                memory=rup_memory,
                control_stack=[None] * 32,
                pc=rup_pc,
    ...
    rup_zero_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    rup_zero_count_state = AbstractRED2MachineState(
    ...
    rup_reverse_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    rup_bad_state = AbstractRED2MachineState(
    ...
    rup_order_state = AbstractRED2MachineState(
    ...
    rup_overflow_state = AbstractRED2MachineState(
    ...
    rup_collision_state = AbstractRED2MachineState(
    ...
    recp_nonhead_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    recp_read = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=4))
    ...
    recp_head_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    recp_read = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=19))
    ...
    recp_reverse_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    recp_read = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=0))
    ...
    recp_zero_machine = AbstractRED2Machine(
        AbstractRED2MachineState(
    ...
    for recp_reverse_env in (28, 27):
        recp_reverse_zero_state = AbstractRED2MachineState(
        ...
        recp_reverse_zero_machine = AbstractRED2Machine(recp_reverse_zero_state)
    ...
    recp_join_state = AbstractRED2MachineState(
        memory=[None] * 40,
    ...
    recp_join_machine = AbstractRED2Machine(recp_join_state)
    for _ in range(7):
    ...
    recp_zero_precedence_state = AbstractRED2MachineState(
        memory=list(recp_zero_memory),
    ...
    recp_bad_state = AbstractRED2MachineState(
    ...
    recp_missing_context_state = AbstractRED2MachineState(
    ...
    recp_collision_state = AbstractRED2MachineState(
    ...
    recp_overflow_state = AbstractRED2MachineState(
    ...
    struct_recon_state = AbstractRED2MachineState(
        memory=[None] * 32,
        control_stack=[None] * 8,
        pc=0,
        fsp=5,
    ...
    struct_recon_result, struct_recon_expected = _run_bounded_to_same_commit(
        AbstractRED2Machine(struct_recon_state), struct_codec
    )
    ...
    struct_recon_mem = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=6))
    assert _packed_memory_read(struct_recon_mem) == struct_recon_expected.memory[6]
    struct_recon_env = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=23))
    assert _packed_memory_read(struct_recon_env) == struct_recon_expected.memory[23]
    struct_recon_control = sim_call(
        SynthesizableRED2Machine, _command(CMD_NOP, address=0)
    )
    ...
    for struct_argument, struct_path in (
        ...
        struct_bind_state = AbstractRED2MachineState(
            memory=[None] * 32,
            control_stack=[None] * 8,
            pc=0,
            fsp=5,
        ...
        struct_bind_result, struct_bind_expected = _run_bounded_to_same_commit(
            AbstractRED2Machine(struct_bind_state), RED2ABICodec()
        )
    ...
    struct_reverse_base = AbstractRED2MachineState(
        memory=[None] * 32,
        control_stack=[None] * 8,
        pc=6,
        fsp=6,
    ...
    struct_collision_state = AbstractRED2MachineState(
        memory=[None] * 32,
        control_stack=[None] * 8,
        pc=0,
        fsp=7,
    ...
    struct_overflow_state = AbstractRED2MachineState(
        memory=[None] * 32,
        control_stack=[None] * CONTROL_WORDS,
        pc=0,
        fsp=5,
    ...
    for struct_fault_opcode in (MuredOpcode.EP, MuredOpcode.APP):
        struct_late_state = AbstractRED2MachineState(
            memory=[None] * 32,
            control_stack=[None] * 8,
            pc=0,
            fsp=7,
    ...
    def run_struct_publication_commit(
        ...
        *,
        ...
        struct_oracle = ConcreteRED2Machine(encoded)
    ...
    def run_selector_join_case(
        ...
        *,
        ...
        selector_oracle = ConcreteRED2Machine(
            encoded,
    ...
    def run_selector_result_atomic_case(
        ...
        result_state = AbstractRED2MachineState(
        ...
        result_oracle = ConcreteRED2Machine(
            encoded_result,
        ...
        for result_address in range(3, 6):
            result_read = sim_call(
                SynthesizableRED2Machine, _command(CMD_NOP, address=result_address)
            )
    ...
    def run_selector_result_struct_case(*, q_value: int) -> object:
        ...
        struct_result_state = AbstractRED2MachineState(
        ...
        struct_result_oracle = ConcreteRED2Machine(
    ...
    def run_selector_result_lambda_case(
        *, q_value: int, argcnt_value: int, body: Word
        ...
        lambda_state = AbstractRED2MachineState(
        ...
        lambda_oracle = ConcreteRED2Machine(
            lambda_encoded,
    ...
    app_shared_state = AbstractRED2MachineState(
    ...
    app_shared_oracle = ConcreteRED2Machine(
        app_shared_encoded,
    ...
    for _ in range(512):
        app_shared_result = sim_call(SynthesizableRED2Machine, _command(CMD_CLOCK))
    ...
    app_shared_second_state = AbstractRED2MachineState(
    ...
    app_shared_second_oracle = ConcreteRED2Machine(
    ...
    unsupported_lambda_state = AbstractRED2MachineState(
    ...
    for _ in range(160):
        unsupported_result = sim_call(SynthesizableRED2Machine, _command(CMD_CLOCK))
    ...
    unknown_result_state = AbstractRED2MachineState(
    ...
    unknown_result_oracle = ConcreteRED2Machine(unknown_result_encoded)
    ...
    unknown_result = None
    for _ in range(160):
        unknown_result = sim_call(SynthesizableRED2Machine, _command(CMD_CLOCK))
    ...
    unknown_parent_read = sim_call(
        SynthesizableRED2Machine, _command(CMD_NOP, address=3)
    )
    ...
    rblock_state = AbstractRED2MachineState(
        memory=rblock_memory,
        control_stack=[None] * 8,
        pc=6,
        fsp=8,
    ...
    rblock_machine = AbstractRED2Machine(rblock_state)
    ...
    rblock_result = sim_call(
        SynthesizableRED2Machine, _command(CMD_NOP, address=9)
    )
    ...
    rblock_bridge_state = AbstractRED2MachineState(
    ...
    rblock_bridge_machine = AbstractRED2Machine(rblock_bridge_state)
    ...
    rblock_bridge_result = sim_call(
        SynthesizableRED2Machine, _command(CMD_NOP, address=19)
    )
    assert (
    ...
    rblock_empty_state = AbstractRED2MachineState(
        memory=list(rblock_memory),
        control_stack=[None] * 8,
        pc=6,
        fsp=8,
    ...
    rblock_collision_state = AbstractRED2MachineState(
        memory=list(rblock_memory),
        control_stack=[None] * 8,
        pc=6,
        fsp=8,
        ...
        *,
        ...
        if expect_fault is not None:
            ...
            parent_read = sim_call(
                SynthesizableRED2Machine, _command(CMD_NOP, address=parent_address)
            )
        ...
        frame_read = sim_call(SynthesizableRED2Machine, _command(CMD_NOP, address=0))
