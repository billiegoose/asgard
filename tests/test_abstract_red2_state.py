...
def test_step_allows_rblock_argcnt_minus_one_sentinel() -> None:
    machine = AbstractRED2Machine.load([Word(MuredOpcode.INT, 1, False)], quantum=1)
...
def test_step_rejects_invalid_machine_counters(field: str, value: int) -> None:
    machine = AbstractRED2Machine.load([Word(MuredOpcode.INT, 1, True)], quantum=1)
...
def test_direct_state_initializes_free_space_once(explicit: bool) -> None:
    from abstract_red2_machine.machine import AbstractRED2MachineState
