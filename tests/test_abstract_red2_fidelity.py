...
def snapshot(machine: AbstractRED2Machine) -> tuple[object, ...]:
    state = machine.state
...
def detailed_snapshot(machine: AbstractRED2Machine) -> tuple[object, ...]:
    state = machine.state
    word = state.memory[state.pc]
    assert word is not None
    return (
...
def test_final_mured_conformance_corpus_matches_chapter3(
    ...
    assert run_source(source, model="abs", quantum=quantum) == run_source(
        source,
        model="thor",
        quantum=quantum,
    )
