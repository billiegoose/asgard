...
from pypeline_red2.oracle import load_compiled_program, run_lockstep
from pypeline_red2.red2_processor import RED2_PROGRAMS_V1
from red2_engine.mured import MuredMachine, MuredOpcode, MuredStopReason, Word
...
def _processor_result_source(loaded) -> str:
    state = loaded.codec.decode_state(loaded.processor.checkpoint())
...
def _forbid_python_red2_evaluation(monkeypatch: pytest.MonkeyPatch) -> None:
    def forbidden(*_args, **_kwargs):
        raise AssertionError("processor execution called the Python RED2 evaluator")

    monkeypatch.setattr(MuredMachine, "step", forbidden)
    monkeypatch.setattr(MuredMachine, "run", forbidden)
    monkeypatch.setattr(MuredMachine, "run_until_suspend", forbidden)
...
def test_nontrivial_pure_program_runs_wholly_on_pypeline_processor(
    name: str,
    source: str,
    expected: str,
    ...
    status = loaded.processor.run_until_suspend(max_clocks=100_000_000)

    assert status == abi.STATUS_COMPLETE, (name, loaded.processor.fault)
    assert loaded.processor.commits > 500, (name, loaded.processor.commits)
    assert _processor_result_source(loaded) == python_result


def test_bounded_loop_recharges_same_processor_and_storage_to_same_result(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = (
        "((Y (LAMBDA (self) (LAMBDA (n) "
    ...
    identities = (id(processor), id(processor.memory), id(processor.control_stack))
    _forbid_python_red2_evaluation(monkeypatch)

    exhaustions = 0
    for _ in range(64):
        status = processor.run_until_suspend(max_clocks=100_000_000)
        assert (id(processor), id(processor.memory), id(processor.control_stack)) == identities
        if status == abi.STATUS_COMPLETE:
            break
        assert status == abi.STATUS_QUANTUM_EXHAUSTED, (status, processor.fault)
        exhaustions += 1
        assert processor.recharge_quantum(8) == abi.STATUS_RUNNING
        assert (id(processor), id(processor.memory), id(processor.control_stack)) == identities
    else:
        raise AssertionError("bounded loop did not complete after repeated recharge")

    assert exhaustions >= 2
    assert processor.commits > 500
    assert _processor_result_source(loaded) == python_result
...
def _python_clock_dots_trace(source: str, dot_limit: int) -> tuple[list[tuple[str, int]], int]:
...
def _processor_clock_dots_trace(
    source: str,
    dot_limit: int,
    ...
    for _ in range(100):
        ...
        if status == abi.STATUS_QUANTUM_EXHAUSTED:
            assert processor.recharge_quantum(500) == abi.STATUS_RUNNING
            continue
        assert status == abi.STATUS_HOST_CALL, (status, processor.fault)
        if processor.pending_host_op == abi.HOST_CLOCK:
            clock += 1_000
            trace.append(("CLOCK", clock))
            if dots >= dot_limit:
                return trace, processor.commits
            result = Word(MuredOpcode.INT, clock)
        elif processor.pending_host_op == abi.HOST_UART_TX:
            argument = loaded.codec.decode_word(
                processor.memory[processor.pending_host_argument - 1]
            )
            assert argument is not None
            assert argument.opcode is MuredOpcode.INT
            assert type(argument.data) is int
            ...
            raise AssertionError(
                f"unexpected clock-dots host op: {processor.pending_host_op}"
            )
        resumed = processor.resume_host_call(loaded.codec.encode_word(result))
        assert resumed in (abi.STATUS_RUNNING, abi.STATUS_QUANTUM_EXHAUSTED)
    raise AssertionError("Pypeline clock-dots did not emit enough dots")
...
def test_clock_dots_effect_order_and_exactly_once_behavior_match_python_red2(
    ...
    _forbid_python_red2_evaluation(monkeypatch)
    processor_trace, processor_commits = _processor_clock_dots_trace(source, dot_limit=4)

    assert processor_trace == python_trace
    assert processor_commits == python_cycles
    assert sum(name == "UART-TX" for name, _ in processor_trace) == 4
