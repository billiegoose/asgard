...
from concrete_red2_machine.machine import ConcreteRED2Machine
from concrete_red2_machine.oracle import (
    RED2_LOCKSTEP_V1,
    architectural_diffs,
    load_compiled_program,
    load_concrete_machine,
    mutate_state,
...
def _reverse_app_underflow_machine() -> AbstractRED2Machine:
...
def test_program_image_is_deterministic_and_preserves_initial_architecture() -> None:
    ...
    codec, image, concrete = load_concrete_machine(left)
    assert concrete.checkpoint() == image.state
    assert concrete.checkpoint() == codec.encode_state(left.state)
...
def test_architectural_diff_localizes_memory_and_control_entries() -> None:
    _codec, image, _concrete = load_concrete_machine(_machine("(+ 2 3)"))
    state = image.state
    memory = list(state.memory)
    memory[0] ^= 1
    control = list(state.control_stack)
    control[2] = 7
    changed = mutate_state(
        state,
        memory=tuple(memory),
...
def test_scheduler_stop_status_is_an_architectural_difference() -> None:
    __codec, image, _concrete = load_concrete_machine(_machine("7"))
    diffs = architectural_diffs(
        image.state,
        image.state,
        oracle_status_value=abi.STATUS_COMPLETE,
        concrete_status_value=abi.STATUS_QUANTUM_EXHAUSTED,
    )
...
def test_typed_fault_code_is_an_architectural_difference() -> None:
    ...
    diffs = architectural_diffs(
        ...
        concrete_status_value=abi.STATUS_FAULT,
        oracle_fault_value=abi.FAULT_INVALID_ADDRESS,
        concrete_fault_value=abi.FAULT_CONTROL_UNDERFLOW,
    )
    assert [(item.field, item.oracle, item.concrete) for item in diffs] == [
...
def test_live_mismatched_typed_fault_reports_exact_fault_divergence(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    original = ConcreteRED2Machine.run_to_commit

    def run_to_commit_with_wrong_fault(
        self: ConcreteRED2Machine,
    ...
    monkeypatch.setattr(
        ConcreteRED2Machine, "run_to_commit", run_to_commit_with_wrong_fault
    )
    result = run_lockstep(_reverse_app_underflow_machine(), max_transitions=4)
    ...
    actual = [
        (item.field, item.oracle, item.concrete)
        for item in result.divergence.diffs
    ]
    assert actual == [
...
def test_host_call_fields_are_architectural_differences() -> None:
    __codec, image, _concrete = load_concrete_machine(_machine("(CLOCK)"))
    state = image.state
    for field, value in (
...
def test_compiled_corpus_has_deterministic_encoded_images(
    ...
    right_codec = RED2ABICodec()
    assert (
        left_codec.encode_program(left) == right_codec.encode_program(right)
    ), category
...
def test_compiled_program_categories_run_in_committed_lockstep(
    ...
    assert result.matches, (
        category, result.divergence.format() if result.divergence else None
    )
...
def test_compiled_definition_image_relocation_and_execution_match() -> None:
    ...
    assert left.image == right.image
    assert left.concrete.checkpoint() == left.image.state

    result = run_lockstep(
        left.abstract,
        max_transitions=512,
        codec=left.codec,
        concrete=left.concrete,
    )
...
def test_divergence_report_names_exact_transition_context_and_field() -> None:
    from concrete_red2_machine.oracle import compare_checkpoint

    machine = _machine("(+ 2 3)")
    codec, _image, concrete = load_concrete_machine(machine)
    concrete.q += 1
    ...
    assert "concrete_microstate=0" in report
    assert "q:" in report
...
def test_io_program_runs_in_lockstep_with_deterministic_host_service() -> None:
    ...
    machine = _machine(
        "(IO-BIND (CLOCK) (LAMBDA (now) "
        "(IO-THEN (UART-TX (MOD now 256)) (IO-RETURN now))))",
        quantum=100,
    )
