...
def test_red2_cli_runs_faithful_expression_corpus_by_default(
    ...
    assert abs_main(["--quantum", str(quantum), "--expr", source]) == 0
...
def test_red2_cli_runs_top_level_definition(capsys: CaptureFixture[str]) -> None:
    ...
    assert abs_main(["--quantum", "20", "--expr", source]) == 0
...
def test_red2_cli_projects_lazy_atomic_struct_field(
    ...
    assert abs_main(["--quantum", "100", "--expr", source]) == 0
...
def test_red2_cli_clock_uses_latest_valid_value_for_io(
    ...
    assert abs_main(["--clock", str(clock), "--expr", source]) == 0
