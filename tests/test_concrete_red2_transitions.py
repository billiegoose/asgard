...
def _loaded(words: list[Word], *, q: int = 8, memory_words: int = 64) -> AbstractRED2Machine:
...
def _processor(machine: AbstractRED2Machine, codec: RED2ABICodec) -> ConcreteRED2Machine:
    return ConcreteRED2Machine(codec.encode_state(machine.state))


def _assert_architecture_matches(
    processor: ConcreteRED2Machine, machine: AbstractRED2Machine, codec: RED2ABICodec
) -> None:
...
def test_stop_is_an_architectural_backward_transition() -> None:
    ...
    machine.state.pc = 1
    machine.state.direction = Direction.B
    # AbstractRED2Machine.step() validates pc again after _stop() increments it, so a
