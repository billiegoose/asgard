...
def assert_diagnostics(machine: AbstractRED2Machine, expected: set[str]) -> None:
...
def test_memory_diagnostics_are_disabled_by_default() -> None:
    machine = AbstractRED2Machine.from_expr(parse_expr("((LAMBDA (x) x) 42)"), quantum=10)
    machine.run()
...
def test_environment_reservation_uses_free_space_atomically(
    ...
    from abstract_red2_machine.machine import GraphEnvironmentCollision
...
def _task3_audit_fixture() -> tuple[AbstractRED2Machine, _SubgraphFrame]:
    machine = AbstractRED2Machine.load(
        [Word(MuredOpcode.INT, 0, True)],
        quantum=10,
        memory_words=64,
...
def test_lazy_struct_construction_reuses_only_dead_suffix_and_preserves_fields(
    ...
    left_machine = AbstractRED2Machine.from_expr(
        result.fields[0],
        quantum=20,
        memory_words=128,
        control_words=32,
    ...
    right_machine = AbstractRED2Machine.from_expr(
...
def test_residual_compaction_capacity_is_transactional(
    ...
    state.memory[2] = Word(MuredOpcode.INT, 1, True)
    machine = AbstractRED2Machine(state, working_memory_limit=working_limit)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ((Word(MuredOpcode.EP, 31, True),), 32, AbstractRED2MachineError),
        (
...
def test_residual_compaction_resets_control_state_and_reuses_evacuated_workspace(
    ...
    machine = AbstractRED2Machine(state, working_memory_limit=24)
...
def test_integrated_reclamation_without_coarse_compaction() -> None:
    ...
    def forbid_coarse_reclamation(machine: AbstractRED2Machine) -> None:
        def forbidden(*_args: object, **_kwargs: object) -> None:
            raise AssertionError(
    ...
    for repetitions in (1, 8, 64):
        ...
        join_machine = AbstractRED2Machine.load(
            [Word(MuredOpcode.INT, 0, True)],
            quantum=10,
            memory_words=32,
            control_words=8,
        ...
        y_state = AbstractRED2MachineState(
            memory=[None] * 12,
            control_stack=[None] * 4,
            pc=1,
            fsp=3,
        ...
        y_machine = AbstractRED2Machine(y_state)
        forbid_coarse_reclamation(y_machine)

        if_state = AbstractRED2MachineState(
            memory=[None] * 16,
            control_stack=[None] * 4,
            pc=3,
            fsp=3,
        ...
        if_machine = AbstractRED2Machine(if_state)
        ...
        equality_machine = AbstractRED2Machine.from_expr(
        ...
        io_machine = AbstractRED2Machine.load(
            [Word(MuredOpcode.INT, 0, True)],
            quantum=10,
            memory_words=32,
            control_words=8,
