...
def test_task6_expt_rejects_huge_nontrivial_exponent_without_committing() -> None:
    ...
    codec = RED2ABICodec()
    processor = _processor(
        MuredMachine(state), codec, scalar_name="EXPT", scalar_op=SCALAR_OP_EXPT
    )
    before_q = processor.q
...
def test_task6_expt_signed_64_bit_boundary_is_bounded() -> None:
    ...
    valid_processor = _processor(
        MuredMachine(valid), codec, scalar_name="EXPT", scalar_op=SCALAR_OP_EXPT
    )
    ...
    overflow_processor = _processor(
        MuredMachine(overflow), codec, scalar_name="EXPT", scalar_op=SCALAR_OP_EXPT
    )
