...
`free_space` is explicit implementation state for the archived physical `fs` role; it is not claimed as an extra thesis execution register. Earlier project revisions called this physical boundary `env_frontier`; current `AbstractRED2MachineState` does not have that field.
...
Long-running IO also has a scheduler-visible coarse safety boundary. After a committed host effect, a low-headroom machine may run through q=0 bounded-result reconstruction and restart the same `AbstractRED2Machine` from a relinearized residual. Roomy machines use `refresh_quantum()` instead. Scheduler statistics distinguish host refreshes, host checkpoints, and quantum-exhaustion recharges; tests require one machine identity and exactly-once ordered effects across all of them.
...
These are narrow implementation reconciliations/bookkeeping choices around incomplete pseudocode and source-language integration; `AbstractRED2Machine.step()` and `run()` do not call the Chapter 3 evaluator.
