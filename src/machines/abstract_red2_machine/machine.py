...
from red2.compiler import compile_lambda
...
class AbstractRED2MachineError(RuntimeError):
    pass


class InvalidAddress(AbstractRED2MachineError):  # noqa: N818
    pass


class GraphEnvironmentCollision(AbstractRED2MachineError):  # noqa: N818
    pass


class ControlStackOverflow(AbstractRED2MachineError):  # noqa: N818
    pass


class ControlStackUnderflow(AbstractRED2MachineError):  # noqa: N818
    pass


class MalformedClosure(AbstractRED2MachineError):  # noqa: N818
    pass


class IllegalTransition(AbstractRED2MachineError):  # noqa: N818
    pass


class CycleLimitExceeded(AbstractRED2MachineError):  # noqa: N818
    pass
...
class AbstractRED2MachineState:
...
class AbstractRED2Machine:
    ...
    def _current_opcode_name(self) -> str | None:
        ...
        except AbstractRED2MachineError:
            return None
    ...
    def step(self) -> AbstractRED2MachineState:
        state = self.state
    ...
    def _allocate_environment_block(self, words: Sequence[Word]) -> int:
        if not words:
            raise AbstractRED2MachineError("environment block must not be empty")
        state = self.state
        frontier = self._frontier()
    ...
    def _materialize_closure_graph(self, closure_address: int) -> tuple[Word, ...]:
            ...
            *,
            ...
            if target.opcode is MuredOpcode.CLOSURE:
                ...
                if output[base] is None:
                    raise AbstractRED2MachineError(
                        "published closure graph contains a hole"
                    )
                first = output[base]
        ...
        def emit_graph(
            ...
            *,
            ...
            if key in visiting:
                raise AbstractRED2MachineError("cyclic μRED graph during publication")
            visiting.add(key)
            try:
                word = self._word(address)
                if word.opcode in {MuredOpcode.APP, MuredOpcode.APP_VAR} or (
        ...
        if not output or any(word is None for word in output):
            raise AbstractRED2MachineError("published closure graph contains a hole")
    ...
    def _audit_typed_subgraph_return_roots(
        ...
        def visit_graph(address: int) -> None:
            ...
            if word.opcode is MuredOpcode.RBLOCK:
                ...
                rup = self._word(cursor)
                if rup.opcode is not MuredOpcode.RUP or rup.data != count:
                    raise AbstractRED2MachineError("audit LETREC requires matching RUP")
            ...
            if word.opcode is MuredOpcode.STRUCT:
                ...
                while cursor < len(state.memory):
                    ...
                    if descriptor.opcode is not MuredOpcode.VAR or descriptor.data != 0:
                        raise AbstractRED2MachineError(
                            "audit STRUCT requires trailing VAR 0"
                        )
                    return
    ...
    def _relinearize_graph(self, address: int) -> tuple[Word, ...]:
        ...
        def emit_pointer_target(address: int) -> int:
            ...
            if (address, True) in visiting:
                raise AbstractRED2MachineError("cyclic μRED result graph")
            target = len(output)
        ...
        def emit_graph(address: int, *, head: bool) -> None:
            ...
            try:
                ...
                if word.opcode is MuredOpcode.RBLOCK:
                    ...
                    if rup.opcode is not MuredOpcode.RUP or rup.data != len(blocks):
                        raise AbstractRED2MachineError(
                            "result LETREC requires matching RUP"
                        )
        ...
        if any(word is None for word in output):
            raise AbstractRED2MachineError("relinearized μRED graph contains a hole")
        return tuple(word for word in output if word is not None)
    ...
    def _write_problem(self, destination: int, words: Sequence[Word]) -> int:
        if not words:
            raise AbstractRED2MachineError("μRED value graph must not be empty")
        state = self.state
        frontier = state.free_space
        last_address = destination + len(words) - 1
        if last_address >= frontier:
    ...
    def resume_host_call(self, result: Word) -> AbstractRED2MachineState:
    ...
    def _relinearize_result_graph(self) -> tuple[Word, ...]:
        ...
        if not self.state.halted:
            raise AbstractRED2MachineError("result graph is available only after halt")
        return self._relinearize_graph(self.state.pc)

    def refresh_quantum(self, quantum: int) -> AbstractRED2MachineState:
        """Reset the budget of a live machine after scheduler-visible progress."""
        if quantum < 0:
            raise ValueError("quantum must be non-negative")
        if self.state.halted:
            raise AbstractRED2MachineError("cannot refresh quantum after halt")
        if self.pending_host_call is not None:
            raise AbstractRED2MachineError(
    ...
    def checkpoint_quantum(self, quantum: int) -> AbstractRED2MachineState:
        ...
        if state.halted:
            raise AbstractRED2MachineError(
                "cannot checkpoint an already halted machine"
            )
        if self.pending_host_call is not None:
            raise AbstractRED2MachineError(
                "cannot checkpoint while a host call is pending"
            )
    ...
    def recharge_quantum(self, quantum: int) -> AbstractRED2MachineState:
    ...
    def run(self, *, cycle_limit: int = 100_000) -> AbstractRED2MachineState:
        if cycle_limit < 0:
            raise ValueError("cycle_limit must be non-negative")
    ...
    def result_expr(self) -> Expr:
        if not self.state.halted:
            raise AbstractRED2MachineError("result is available only after halt")
    ...
    def _decompile_ep_value(
        ...
        while True:
            if target_address in seen:
                raise AbstractRED2MachineError("cyclic result EP environment chain")
            seen.add(target_address)
            target = self._word(target_address)
        ...
        if target.opcode is MuredOpcode.INT:
            if type(target.data) is not int:
                raise AbstractRED2MachineError("result INT requires an integer value")
        ...
        if target.opcode in {
            ...
            if type(target.data) is not str or target.data == "":
                raise AbstractRED2MachineError("result symbol requires a symbol name")
    ...
    def _decompile(
            ...
            for opcode, argument_data in reversed(argument_entries):
                ...
                if inline_word.opcode is MuredOpcode.EP:
                    ...
                    if type(inline_word.data) is not int:
                        raise AbstractRED2MachineError(
                            "result INT requires an integer value"
                        )
                    ...
                    if type(inline_word.data) is not str or inline_word.data == "":
                        raise AbstractRED2MachineError(
                            "result symbol requires a symbol name"
                        )
        ...
        if word.opcode is MuredOpcode.RBLOCK:
            ...
            rup = self._word(cursor)
            if rup.opcode is not MuredOpcode.RUP or rup.data != len(blocks):
                raise AbstractRED2MachineError("result LETREC requires matching RUP")
            letrec_path = letrec_path | {cursor}
        ...
        if word.opcode is MuredOpcode.STRUCT:
            if type(word.data) is not str or word.data == "":
                raise AbstractRED2MachineError("result STRUCT requires a tag name")
