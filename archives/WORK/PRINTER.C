...
#define MAX_PRINT_DEPTH    45       /* value is determined experimentally */
...
extern symbol *nil;
extern symbol *pair;
extern symbol *var_out_of_scope;
extern jmp_buf abort_context;

#define BUFFER_SIZE     4096     /* Size of the output buffer */
...
va_dcl
{
   ...
   va_end(args);
   if (prin_index > &buffer[end_of_buffer]) {
...
int depth;
{
   ...
   if ((i->class & PAIR) && (i->op.sym == pair)) {
   ...
   else if (i->type == EX_LAMBDA) {
      while(i->type == EX_LAMBDA) {
      	(++bindings)->sym = var_out_of_scope;
      	i++;
      }
      ...
      if (parens) sprint("(");
		if (i->type == EX_LIST) {
			...
			int var;
			(++bindings)->sym = var_out_of_scope;
			var = bindings - orig_stack;
			first = 1;
			while (ptr->type != STOP) {
				if (ptr->type == MARKER) ptr = ptr->op.addr;
				else {
					if (((ptr->op.addr+2)->op.index == var) &&
					    (ptr->op.addr->type == EX_VAR) &&
					    (!(ptr->op.addr->class & EXISTS))) {
						if (first) {
							sprint("LAMBDA (");
							first = 0;
						}
						else sprint(" ");
						sprint((ptr->op.addr+3)->op.sym->print_name);
					}
					++ptr;
				}
			}
			if (!first) {
				sprint(") ");
				print_express(i+1, bindings, depth+1);
				sprint(")");
				return;
			}
		}	
      if (i->type == LAMBDA) {
         ...
         while (((i->type == LAMBDA) && !(i->class & PAIR)) ||
					 (i->type == EX_LAMBDA)) {
				if (i->type == EX_LAMBDA) (++bindings)->sym = var_out_of_scope;
				else { 
	            (++bindings)->sym = i->op.sym;
   	         if (! first) sprint(" ");
      	      sprint("%s", i->op.sym->print_name);
      ...
      /* Print the spine */
      while (i >= finished) {
         if (!first && (i->type != RUP) && (i->type != EX_LAMBDA) &&
					(i->type != EX_LIST) && (i->type != NOOP))
				sprint(" ");
         print_exp(i, bindings, depth+1);
         i--;
         first = 0;
...
int depth;
{
   ...
   switch (i->type) {
      ...
      case RUP:      break;   
		...
		case EX_VAR:
			{
				control *limit = bindings - (i->op.addr+2)->op.index;
				control *temp = bindings;
				symbol *pname;

				if (i->op.addr->type != EX_VAR) {
					/* Because exvar in a BIND may have been reset */
					if (i->op.addr->type != UBV) {
						foutstring_ns(stdscr, "\n\n\rUnexpectedly, an EX_VAR is not a UBV at (%p) = ",
							i->op.addr);
						print_node(stdscr, i->op.addr);
						print_exp(i->op.addr, bindings, depth+1);
						return;
					}
					else {
						i->type = VAR;
						i->op.index = (bindings - orig_stack) - i->op.addr->op.index;
						print_exp(i, bindings, depth+1);
						return;
					}
				}

				pname = (i->op.addr+3)->op.sym;
				if (pname == NULL) {
					i->type = VAR;
					i->op.index = (bindings - orig_stack) - (i->op.addr+2)->op.index;
					print_exp(i, bindings, depth+1);
					return;
				}					
				while(temp > limit) {
     ...
     default:  sprint("   Printer Error: Unknown type in output at (%p): %d  ", 
                        i, i->type);
   }
}
