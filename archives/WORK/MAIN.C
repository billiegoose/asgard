...
#define  VERSION  "0.4"             /* System version number */
...
node		*heap;			/* entrance to heap area for reduced UNIFY graphs  */
...
reset		*reset_ptr;		/* pointer to top of reset list */
...
extern char *file_name, history_temp[];
extern node *mem;
...
char *argv[];
{
...
#ifdef IBM
	MEM_SIZE = 10000;
   mem = (node huge *)halloc(MEM_SIZE, sizeof(node));
#endif
#ifdef SUN
	MEM_SIZE = 100000;
...
#ifdef IBM
	HEAP_SIZE = 5000;
   heap_space = (node huge *)halloc(HEAP_SIZE, sizeof(node));
#endif
#ifdef SUN
	HEAP_SIZE = 5000;
   heap_space = (node *) malloc(HEAP_SIZE * sizeof(node));
#endif
   if (mem == NULL) {
      fprintf(stderr, "\nUnable to allocate memory for HEAP.");
      exit(1);
   }
#ifdef IBM
	CONTROL_SIZE = 1000;
   constack = (control huge *)halloc(CONTROL_SIZE, sizeof(control));
#endif 
#ifdef SUN
	CONTROL_SIZE = 10000;
...
#ifdef IBM
	RESET_SIZE = 1000;
   reset_list = (reset huge *)halloc(RESET_SIZE, sizeof(reset));
#endif 
#ifdef SUN
	RESET_SIZE = 10000;
   reset_list = (reset *) malloc(RESET_SIZE * sizeof(reset));
#endif
   if (reset_list == NULL) {
      fprintf(stderr, "\nUnable to allocate memory for RESET_LIST.");
      exit(1);
   }
   ...
   if (terminal) {
      ...
      foutstring(stdscr, "\n\r%lu graph nodes, %lu control stack, %lu reset list, %lu heap nodes", 
                  MEM_SIZE, CONTROL_SIZE, RESET_SIZE, HEAP_SIZE);
      wrefresh(stdscr);            
   }
...
FILE *in;
{
   ...
   switch (expr_type) {
      ...
      case LAM_RED:  /* reduce current focus expression */        
            if (focus != NONE) {
               if (focus == OLD) {
               	int temp = get_history_string(output_history.front,
											&output_history);
               	copy_history_string(temp, &history_temp[0], &output_history);
						exp = &history_temp[0];               	
#ifdef DEBUG_PARSER
					   if (debug) {
					      foutstring(stdscr, "\n\rInput string: \"%s\"", exp);
...
char *com, *modifier;
{
   ...
   if (stricmp(com, "EXIT") == 0) {
      ...
      hfree((void huge *) reset_list);
      hfree((void huge *) heap_space);
#endif
#ifdef SUN
      free(mem);
      free(constack);
   ...
   else if (stricmp(com, "DUMP") == 0) dump();
...
int flag;
{
	if (total_reds != reductions)
		foutstring(stream, "\n\r%lu reductions, this step.", reductions);
   ...
   if (flag) {
      ...
      foutstring(stream, "\n\r   max aux size: %lu", (unsigned long) (initial_aux - max_aux));
		foutstring(stream, "\n\r max reset size: %lu", (unsigned long) (max_reset - reset_list));
		foutstring(stream, "\n\r  max heap size: %lu", (unsigned long) (heap - heap_space));
      wrefresh(stream);    
   }
}
