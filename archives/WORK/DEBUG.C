...
extern FILE *logfile;
extern node *ws, *result, *fs, *problem, *ex_ptr, *heap, *high;
...
dump()
{
   ...
   outstring(stdscr, "\n\rEX-LIST AREA:");
   print_mem(stdscr, ex_ptr, &heap_space[HEAP_SIZE-1]);
}
...
node *i;
{
   ...
   switch (i->type) {
		...
		case EX_LIST:
			{
				node *ptr = i->op.addr;
				foutstring(stream, "EX_LIST (%p) ", i->op.addr);
				while (ptr->type != STOP) {
					if (ptr->type == MARKER) ptr = ptr->op.addr;
					else {
						if ((ptr->op.addr+3)->op.sym != NULL)
							foutstring(stream, " %s",
								(ptr->op.addr+3)->op.sym->print_name);
						ptr++;
					}
				}
				break;
		...
		case EX_VAR:	foutstring(stream, "EX_VAR %p", i->op.addr); break;
      ...
      case CLOSURE:  foutstring(stream, "CLOSURE (%p) {P: %p, E: %p}",
								i->op.addr, i->op.addr->op.addr, (i->op.addr+1)->op.addr);
                     break;                    
