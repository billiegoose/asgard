/* Primitive Operators for the Head Order Reduction System
/* Developed by Mike Hilton, 10 Jan 1990
/* Copyright 1989, 1990  Mike Hilton,  All Rights Reserved
/*
...
void prim_floor(), prim_ceiling(), prim_expt();
...
void prim_char_int(), prim_int_char(), prim_charp();
...
struct primtab primtable [] = {
   ...
   {"char->int", PRIM_1, prim_char_int},
   {"int->char", PRIM_1, prim_int_char},
   {"char?", PRIM_1, prim_charp},
   {"string?", PRIM_1, prim_stringp},
...
void install_primitives()
{
   ...
   string = symbol_lookup("$$string", SYM);

#ifdef IBM
   prelude = PRELUDEFILE;
#endif
#ifdef SUN
   prelude = getenv(PRELUDEFILE);
#endif

   if (load_file(prelude, line_number, file_name)) {
      /* Mark the CONS instruction with a PAIR indicator. */
      node *ptr = ((symbol_lookup("cons", SYM))->def.user) + 2;
      ptr->class = ptr->class | PAIR;
      /* Mark the CONS-STRING instruction with a PAIR indicator. */
      ptr = ((symbol_lookup("cons-string", SYM))->def.user) + 2;
      ptr->class = ptr->class | PAIR;
   }
      ...
      case RESULT:   
         {
            ...
            if (reductions == red_limit) {
               if (debug) {
                  outstring(stdscr, "\n\OR primitive -- NOT FIRING, redlimit");
                  wrefresh(stdscr);
               }
               return;
            }
...
void prim_equal_star()
{
   ...
   if (reductions == red_limit) {
      ...
      (arg1+1)->class = HEAD; (arg1+1)->type = PRIM_0;
      (arg1+1)->op.sym = equal;
      pc = res - 1;
      return;
   }
   ...
   if ((arg1->type != PTR) || (arg2->type != PTR)) {
      if (debug) {
         outstring(stdscr, "\n\r  FIRING false, args not both pointers");
         wrefresh(stdscr);
      }
      make_result(res, false);
      return;
   ...
   if ((arg1->op.addr->type == LAMBDA) && (arg1->op.addr->class & PAIR) &&
      ...
      while (len1-- > 0) {
         ...
         *(++ws) = *(++arg1); t1 = ws;
         ...
         if (!atomic(t2) || (t2->type == VAR)) wrap_lambdas(lambdas, t2);
         if (!atomic(t1) || (t1->type == VAR)) wrap_lambdas(lambdas, t1);
      }
      (++ws)->class = CONTROL; ws->type = JOIN; ws->op.addr = res;
      res->class = HEAD;
...
int reduced;
{
   ...
   switch ((*arg1)->type) {
      case CHARAC:   return(((*arg1)->op.charval == (*arg2)->op.charval));
...
void prim_char_int()
{
   node  *arg = primitive-1;
   
   if (reductions == red_limit) return;
   if (arg->type != CHARAC) return;

   arg->class = HEAD;
   arg->type = INT;
   arg->op.intval = arg->op.charval;
   ws = arg;
   ++reductions;
}
...
void prim_charp()
{
   ...
   if (arg->type == CHARAC) arg->op.sym = true;
   else arg->op.sym = false;
   arg->class = HEAD;
   arg->type = SYM;
   ws = arg;
   ++reductions;
...
make_result(res, value)
node *res;
symbol *value;
{
    res->class = HEAD; res->type = SYM; res->op.sym = value;
    ++reductions;
    pc = res - 1;
    ws = res;
    mode = RESULT;
