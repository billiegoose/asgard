...
/* Developed by Mike Hilton, 11 Jan 1990
      ...
      if (arg2->type == INT) {
         ...
         switch (operator) {
            ...
            case MIN:   if (int1 <= int2) iresult = int1; else iresult = int2;
                        break;
            case EXPT:  iresult = pow(int1*1.0, int2*1.0); break;
         }
         ...
         switch (operator) {
            ...
            case MIN:   if (int1 <= flo2) {
                        ...
                        break;
             case EXPT: fresult = pow(int1*1.0, flo2); break;
         }
      ...
      if (arg2->type == INT) {
         ...
         switch (operator) {
            ...
            case MIN:   if (flo1 <= int2) fresult = flo1; 
                        ...
                        break;
            case EXPT:  fresult = pow(flo1, int2*1.0); break;
         }
         arg2->class = HEAD; arg2->type = FLOAT; arg2->op.floval = fresult;
         ws = arg2;
      }
...
int operator;
{
   ...
   switch (operator) {
      ...
      case SQRT:  if (arg->type == INT) {
                     arg->type = FLOAT;
                     arg->op.floval = sqrt(arg->op.intval*1.0);
                  }
                  else {
