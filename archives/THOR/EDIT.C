...
/* Developed by Mike Hilton, 27 Sep 1989
         ...
         case CTRL_Y:   /* Yank from input history buffer, cycling through history */
                     {
                        ...
                        insert_string_from_index(win, &(input_history.queue[yank_start]), &index, &eob);
                        wrefresh(win);
                        break;
                     }
         ...
         case CTRL_R: /* Yank from output history */
                     {
                        ...
                        if (last_ch != CTRL_R) {
                           ...
                           move_cursor_to_char(win, old_index, index);
                           index = old_index;
                           delete_buffer_from_index_to(win, index, strlen(&(output_history.queue[yank_start]))+index, &eob);
                        ...
                        insert_string_from_index(win, &(output_history.queue[yank_start]), &index, &eob);
                        wrefresh(win);
                        break;
                     }
