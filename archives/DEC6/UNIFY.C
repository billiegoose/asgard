...
void prim_unify_star()
{
   ...
   if (arg1->type == EP) {
   	...
   	if (((arg2->type == PTR) || (arg2->type == CLOSURE)) &&
			 occurs(arg1->op.addr->op.index, arg2->op.addr, env, 0)) {
   		make_result(res, false);
   		return;
   	}
   	else {
   ...
   if (arg2->type == EP) {
   	...
   	if (((arg1->type == PTR) || (arg1->type == CLOSURE)) &&
			 occurs(arg2->op.addr->op.index, arg1->op.addr, env, 0)) {
   		make_result(res, false);
   		return;
   	}
   	else {
...
*/
int occurs(var, graph, env, lambdas)
int var, lambdas;
node *graph, *env;
{
	reset *res = reset_ptr;
	if (occurs1(var, graph, env, lambdas)) {
		undo_bindings(res);
		return(TRUE);
	}
	else return(FALSE);
	...
	if (graph->type == VAR) {
		...
		if (value->type == UBV) {
			...
			if (value->op.index  > var) {
				...
				if (!(value->class & EXISTS)) {
					value->class = value->class | EXISTS;
					(value+2)->op.index = var;
					(++reset_ptr)->val = value->op.index;
					reset_ptr->addr = value;
				}
				else if ((value+2)->op.index > var)
					(value+2)->op.index = var;
			}
		}
		if (atomic(value)) {
