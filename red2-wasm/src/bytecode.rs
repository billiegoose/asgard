...
impl Program {
    pub fn decode(bytes: &[u8]) -> Result<Self, Red2Error> {
        ...
        let word_count = read_u32(bytes, 12)? as usize;
        let literal_count = read_u32(bytes, 16)? as usize;
        let meta_count = read_u32(bytes, 20)? as usize;
        let _meta_size = read_u32(bytes, 24)? as usize;
        let mut offset = HEADER_SIZE;
        let mut raw = Vec::with_capacity(word_count);
        for _ in 0..word_count {
            ...
            raw.push((opcode, head, kind, data));
        ...
        let mut instructions = Vec::with_capacity(word_count);
        for (opcode, head, kind, data) in raw {
...
mod tests {
    ...
    fn parses_sample_program() {
        let program = Program::decode(&sample_program()).unwrap();
        assert_eq!(program.entry, 0);
        assert_eq!(program.instructions.len(), 2);
        assert_eq!(program.instructions[0].opcode, Opcode::Int);
    ...
    fn rejects_bad_magic() {
        ...
        let error = Program::decode(&bytes).unwrap_err();
