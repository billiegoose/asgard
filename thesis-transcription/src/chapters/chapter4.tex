...
\longmapsto \mathrm{con1}\\
&\vdots\\
&\langle\!\langle F,\phi,q_1,\rho',c,e,r\rangle\!\rangle
\qquad \mathrm{con2}
\end{align*}
...
In addition to these new registers, \(\mu\)RED inherits four registers from the SSM. The data registers d, \(\phi\), and q play the same role in \(\mu\)RED as they do in the SSM. The address register
...
The benefit of constant time variable access is outweighed by the expense (in both time and space) of placing a copy of p in every closure. \(\mu\)RED takes a different approach to managing the redex store --- a single copy of the tree is built in the environment area and
