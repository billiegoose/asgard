...
\section{The Pure Untyped \(\lambda\)-Calculus}
...
\begin{enumerate}
\item \(\underline{\alpha\text{-conversion}}\):
  \(\lambda x.e \longleftrightarrow \lambda y.[y/x]e\), where \(y \notin \fv(e)\).
\item \(\underline{\beta\text{-conversion}}\):
  \((\lambda x.e_1)e_2 \longleftrightarrow [e_2/x]e_1\).
\item \(\underline{\eta\text{-conversion}}\):
...
\[
\underline{\beta\text{-Reduction:}}\quad
(\lambda x.e_1)e_2 \longrightarrow [e_2/x]e_1 .
\]
The notation \(\overset{*}{\longrightarrow}\) will be used to denote the reflexive,
transitive closure of \(\longrightarrow\) including \(\alpha\)-conversions, and
\(\overset{n}{\longrightarrow}\) will denote \(n\) applications of the
...
\[
\begin{aligned}
\underline{(\lambda x.\lambda y.y\ x\ x)(\lambda z.((\lambda w.w)\ z))}
&\longrightarrow\\
(\lambda y.y\ \underline{(\lambda z.((\lambda w.w)\ z))}\ (\lambda z.((\lambda w.w)\ z)))
&\longrightarrow\\
(\lambda y.y\ (\lambda z.z)\ \underline{(\lambda z.((\lambda w.w)\ z))})
&\longrightarrow\\
...
\[
\begin{aligned}
(\lambda x.\lambda y.y\ x\ x)(\lambda z.\underline{((\lambda w.w)\ z)})
&\longrightarrow\\
\underline{(\lambda x.\lambda y.y\ x\ x)(\lambda z.z)}
&\longrightarrow\\
(\lambda y.y\ (\lambda z.z)\ (\lambda z.z)).
\end{aligned}
\]
...
\emph{sharing} the results of some subgraph reductions.
...
\caption[Representation of expressions in Wadsworth's graph notation]{The expression \(((\lambda x.xx)(\lambda y.y))\) represented in
...
\caption[Contraction of a \(\beta\)-redex in Wadsworth's reduction system]{The graph after contraction of the \(\beta\)-redex. The dashed arcs
