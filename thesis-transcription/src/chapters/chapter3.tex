...
(THOR). THOR is a \emph{conservative extension} of the pure \lambdacalculus, which
...
\[
\begin{array}{rcl}
\langle expression\rangle &::=& \langle exp\rangle^\dagger\\
...
\langle abstraction\rangle &::=&
  \THOR{(LAMBDA (}\langle symbol\rangle^\dagger\THOR{) }
  \langle expression\rangle\THOR{)} \mid{}\\
&& \THOR{(LETREC (}\langle binding\text{-}pair\rangle^\dagger\THOR{) }
...
\end{array}
\]
A superscript \(\dagger\) means one or more occurrences and a superscript
\(*\) means zero or more occurrences. A lexical description of each constant
type is omitted; when a constant is presented, its type will be obvious.

The symbol which is the first component of a structure is called its \emph{tag}. The
...
associations together form a \emph{definition context} within which expressions are
...
not necessarily hold for THOR because the value of symbolic constants depends
...
symbols are considered to be \emph{symbolic constants}. It may appear that this results
in some loss of expressive power, but that is really not the case. Through the
use of \(\equiv\), symbolic constants perform the same role as free variables (i.e.,
...
The semantics of THOR are presented in terms of \emph{configurations}, which are
...
The \(\delta\) mapping is the definition context created through use of \(\equiv\) to
...
complete by extending them to return the value \emph{bottom} (\(\bot\)) everywhere they
...
referred to as the \emph{quantum}. When this number is exhausted (i.e., \(q=0\)), I
...
pronounced as ``reduces to.'' A reduction rule consists of a \emph{conclusion} (the
reduction above the line), zero or more \emph{premises} (any reductions below the
line), and zero or more \emph{application conditions} (also below the line). The
...
\end{array}
\]
\caption[THOR Translation Rules]{Translation from THOR to the constructs used in the semantic rules.
...
\cfg{\delta}{\phi}{0}{\rho}{e_t}\redsto
\cfg{\delta}{\phi}{0}{\rho}{e'_t}\\
\cfg{\delta}{\phi}{0}{\rho}{e_f}\redsto
\cfg{\delta}{\phi}{0}{\rho}{e'_f}
\end{gathered}
}{
\cfg{\delta}{\phi}{q_0}{\rho}{\THOR{(IF }e_c\ e_t\ e_f\THOR{)}}\redsto
\cfg{\delta}{\phi}{q_1}{\rho}{\THOR{(IF }e'_c\ e'_t\ e'_f\THOR{)}}
}
...
(((LAMBDA (X) ((LAMBDA (Y) (X (Y Y)))
              (LAMBDA (Y) (X (Y Y)))))
  (LAMBDA (FACT N) (IF (= N 0) 1 (* N (FACT (1- N))))))
 3) \(\longrightarrow\)
...
(IF (= 3 0)
    ...
    (* 3 ((LAMBDA (Y)
          ...
          (1- 3))))
\end{alltt}
The expressions in this sequence get complicated quickly; if you were shown only
the third expression, would you be able to clearly identify what was going on?
(Consider the case of several interacting recursive functions.) Now examine the
reduction sequence for the \THOR{Y} primitive:
\begin{alltt}
((Y (LAMBDA (FACT N) (IF (= N 0) 1 (* N (FACT (1- N)))))) 3)
  \(\xrightarrow{2}\) ((LAMBDA (N)
        (IF (= N 0)
            1
            (* N ((Y (LAMBDA (FACT N)
                       (IF (= N 0) 1 (* N (FACT (1- N))))))
...
\cfg{\delta}{\phi+n}{0}{\rho'}{e'_i},\ \forall i\in\{0\ldots n\}
}{
...
\[
\frac{
\cfg{\delta}{\phi+n}{0}{\rho'_0}{e_i}\redsto
\cfg{\delta}{\phi+n}{0}{\rho'_0}{e'_i},\ \forall i\in\{0\ldots n\}
}{
...
\]
\[
\rho'_0=\rho_0+[\THOR{(UBV }\phi+1\THOR{)},\ldots,\THOR{(UBV }\phi+n\THOR{)}],
...
been associated with expressions by the use of the \(\equiv\) mechanism. This is the
...
factorials:
\begin{alltt}
fact \(\equiv\) (lambda (n) (if (= n 0) 1 (* n (fact (1- n)))))
\end{alltt}
...
\cfg{\delta}{\phi}{q_0-1}{\rho}{\THOR{(AND (EQUAL? }\lambda^n.e_0\ \lambda^n.e'_0\THOR{) }\ldots\THOR{ (EQUAL? }\lambda^n.e_i\ \lambda^n.e'_i\THOR{))}}\redsto
\cfg{\delta}{\phi}{q_1}{\rho}{e}
}
\quad q_0>0
$}
...
\begin{alltt}
(equal?  1 1) \(\longrightarrow\)  true
(equal?  (a b) (a b)) \(\longrightarrow\)  true
(lambda (x) (equal?  (a 1) (x 1))) \(\longrightarrow\)  (lambda (x) (equal?  a x))
(equal?  (lambda (a) a) (lambda (b) b)) \(\longrightarrow\)  true
(equal?  (lambda (x) 1) (lambda (x) x)) \(\longrightarrow\)  false
(lambda (x) (equal?  [1 x 3] [1 2 3])) \(\longrightarrow\)
        (lambda (x) (equal?  x 2))
(equal?  \{node 2 (+ 1 4)\} \{node 2 5\}) \(\longrightarrow\)  true
\end{alltt}
